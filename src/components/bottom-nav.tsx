...
export function BottomNav() {
  return (
    ...
    >
      <div className="mx-auto flex max-w-md items-start justify-between px-10 pt-3 pb-2">
        {tabs.map(({ to, label, icon: Icon }) => (
          <Link
            key={to}
            to={to}
            activeOptions={{ exact: true }}
            className="flex flex-col items-center gap-1 py-1 text-muted-foreground data-[status=active]:text-accent"
          >
